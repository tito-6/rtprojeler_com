...
const ArabianRanchesConstructionUpdates = () => {
  const { t } = useTranslation("common");

  const projelertartDate = useMemo(() => new Date("2024-11-01").getTime(), []);
    ...
    const fetchConstructionProgress = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/api/arabian-ranches-construction-updates`
        );
  ...
  const radialOptions = useMemo(
    () => ({
        ...
        radialBar: {
          ...
          dataLabels: {
            name: {
              ...
              color: "#333",
              fontSize: "17px",
              text: t("arabianRanchesConstructionUpdates.constructionProgress"),
            },
            value: {
      ...
      labels: [t("arabianRanchesConstructionUpdates.completed")],
    }),
    [t]
  );
  ...
  return (
    ...
    >
      <div className="radial-chart">
        ...
        >
          {t("arabianRanchesConstructionUpdates.arabianRanchesConstructionProgress")}
        </h2>
        <ReactApexChart
      ...
      <div className="real-time-chart mt-10">
        <h2 className="text-center text-3xl font-bold mb-6 dark:text-gray-100 text-gray-900">
          {t("arabianRanchesConstructionUpdates.realTimeArabianRanchesProgress")}
        </h2>
        <ReactApexChart
...
export default React.memo(ArabianRanchesConstructionUpdates);
